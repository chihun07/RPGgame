...
class Player:
                ...
                for mounted_item_name, mounted_item in self.Mounted_inventory.items():
                    if mounted_item_name.split('_')[0] == item_name.split('_')[0]:
                        if int(available_item["grade"]) > int(mounted_item["grade"]):
                            ...
                            number_grade = item_name.split('_')[-1]
                            if item_name.split("_")[0] == "머리":
                                helmet = Armor(f"{item_name}", 20*number_grade, 0, 0.2*number_grade, 0, 0.02*number_grade)  # 머리 갑옷: 최대 체력 20 증가
                            elif item_name.split("_")[0] == "가슴":
                                chestplate = Armor(f"{item_name}", 50*number_grade, 0, 0.5*number_grade,0,0.02*number_grade)
                            elif item_name.split("_")[0] == "손":
                                leggings = Armor(f"{item_name}", 10*number_grade, 0, 0.1*number_grade,0,0.02*number_grade)
                            elif item_name.split("_")[0] == "다리":
                                boots = Armor(f"{item_name}", 30*number_grade, 0, 0.3*number_grade,0,0.02*number_grade)
                            elif item_name.split("_")[0] == "발":
                                gloves = Armor(f"{item_name}", 10*number_grade, 0, 0.1*number_grade,0,0.02*number_grade)
    ...
    #갑옷 장착 함수
class Armor:
    def __init__(self, name, Max_HP_bonus=0, STR_bonus=0, VIT_bonus=0, Critical_bonus=0, Avoid_bonus=0):
        self.name = name
        self.Max_HP_bonus = Max_HP_bonus
        self.STR_bonus = STR_bonus
        self.VIT_bonus = VIT_bonus
...
def attack(player, monster):
    ...
    if random.random() < player.Critical:
        ...
        print("Critical Hit!!!")
    # 데미지 적용
    AKT_rounded = round(AKT, 1)
    monster.HP -= AKT_rounded
    delayed_print(f"{player.Name}이(가) {monster.Name}에게 {AKT}의 피해를 입혔습니다!")
...
def mon_attack(player, monster):
    ...
    # 데미지 적용
    AKT_rounded = round(AKT, 1)
    player.HP -= AKT_rounded
    delayed_print(f"{monster.Name}가 플레이어 {player.Name}에게 {AKT}의 피해를 입혔습니다!")
